...
- 打包的 `gsd-workflow` MCP server 默认只暴露 canonical workflow tool 名称。只有仍在调用旧 alias 名称的客户端才需要设置 `GSD_MCP_ADVERTISE_ALIASES=1`。原生进程内 GSD 工具只注册 canonical 名称，没有对应的 alias 开关。
