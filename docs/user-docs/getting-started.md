...
Local runtime/cache paths include `.gsd/gsd.db*`, `.gsd/state.json`, `.gsd/runtime/`, `.gsd-worktrees/`, and `.gsd-backups/`. GSD manages `.gitignore` for these paths by default. `.gsd/state.json` is the state contract projection used by external readers such as GSD Workbench; do not edit or commit it. `.gsd-backups/` holds migration snapshots left over from older versions; current releases write no new ones, so the directory can be removed once you no longer need its history.
