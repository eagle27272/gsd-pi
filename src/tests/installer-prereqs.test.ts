...
test('runtime checks fall back when CommonJS cannot require ESM output', () => {
  ...
  assert.equal(runtimeChecks.MIN_NODE_VERSION, '24.20.0')
  assert.deepEqual(runtimeChecks.checkNodeVersion('24.19.9'), { ok: false, actualVersion: '24.19.9' })
  assert.deepEqual(runtimeChecks.checkNodeVersion('24.20.0'), { ok: true })
