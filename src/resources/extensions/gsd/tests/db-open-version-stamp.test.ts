...
//   (b) opening a newer-than-supported database throws the typed SchemaTooNewError with the
...
test("v45→v49 migration stamps application_id, user_version, and schema_version (with backup)", () => {
  ...
  assert.ok(db.prepare("PRAGMA table_info(milestones)").all().some((column) => column["name"] === "horizontal_checklist"));
