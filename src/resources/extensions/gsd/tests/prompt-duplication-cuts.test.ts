...
function makeBase(prefix: string): string {
  ...
  mkdirSync(join(base, ".gsd", "phases", canonicalPhaseDirName("M001", "Prompt Cuts")), { recursive: true });
  return base;
}
...
function writeRoadmapAndPlan(base: string): void {
  writeFileSync(
    join(base, ".gsd", "phases", canonicalPhaseDirName("M001", "Prompt Cuts"), "01-ROADMAP.md"),
    [
      "# M001 Roadmap",
      "## Slices",
  ...
  writeFileSync(
    join(base, ".gsd", "phases", canonicalPhaseDirName("M001", "Prompt Cuts"), "01-01-PLAN.md"),
    [
      "# S01 Plan",
      "",
...
function writeTaskSummary(base: string, options?: { blocker?: boolean; repeatedNarrative?: string }): void {
  ...
  writeFileSync(
    join(base, ".gsd", "phases", canonicalPhaseDirName("M001", "Prompt Cuts"), "S01-T01-SUMMARY.md"),
    [
      "---",
      "id: T01",
      "parent: S01",
...
test("execute-task rendering makes memory_query and template disk reads fallback-only", async (t) => {
  ...
  writeFileSync(
    join(base, ".gsd", "phases", canonicalPhaseDirName("M001", "Prompt Cuts"), "S01-T01-PLAN.md"),
    "# T01 Plan\n\nDo the prompt edit.\n",
  );
...
test("complete-slice renders task summary excerpts without full summary bodies", async (t) => {
  ...
  assert.match(prompt, /On-demand.*read `\.gsd\/phases\/01-prompt-cuts\/S01-T01-SUMMARY\.md` only when this excerpt is absent\/truncated/s);
