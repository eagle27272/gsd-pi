...
test("generic executor refuses the retired import-kernel operation types", (t) => {
  openFixture(t);
  ...
  assert.throws(
    ...
    /import\.apply is a retired operation type/,
  );
  assert.throws(
    () => executeDomainOperation(request({
      operationType: "import.forward_repair",
      idempotencyKey: "retired/forward-repair",
    }), () => {
      mutationCalls += 1;
      return mutation();
    }),
    /import\.forward_repair is a retired operation type/,
  );
...
test("generic Domain Operation cannot select Authority Epoch advancement or invoke cutover", (t) => {
  ...
  assert.throws(
    ...
    /authority\.cutover is a retired operation type/,
  );
  assert.deepEqual(durableSnapshot(), before);
});
...
test("receipt component verification still rejects request identity drift", (t) => {
  openFixture(t);
  const applyRequest = request({ idempotencyKey: "receipt-components/original" });
  const applyMutation = mutation();
  const applied = execute(applyRequest, () => applyMutation);

  assertDomainOperationReceiptComponents(applied, applyRequest, applyMutation);
  assert.throws(
    () => assertDomainOperationReceiptComponents(
      applied,
      { ...applyRequest, idempotencyKey: "receipt-components/drifted" },
      applyMutation,
    ),
