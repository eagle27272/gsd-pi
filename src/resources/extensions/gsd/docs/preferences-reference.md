...
- `git`: configures GSD's git behavior. All fields are optional — omit any to use defaults. Keys:
  ...
  - `manage_gitignore`: boolean — when `false`, GSD writes no ignore rules at all. GSD never touches the tracked `.gitignore`; baseline patterns go to `.git/info/exclude`, which stays local to your clone. Set this to `false` if you'd rather declare GSD's ignores yourself. Default: `true`.
