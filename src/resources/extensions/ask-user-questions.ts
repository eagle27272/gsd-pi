...
// Prevents duplicate question dispatches when the LLM calls ask_user_questions
// multiple times with the same questions in a single turn. Keyed by full
// canonicalized payload (id, header,
...
export default function AskUserQuestions(pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Preview content is rendered in a fixed-height panel (max ~20 lines visible). Keep previews concise — show the most relevant snippet, not exhaustive examples. Longer content stays fully accessible: the user scrolls it with Ctrl+U/Ctrl+D (or PgUp/PgDn outside a multiplexer), with '▲/▼ N more' indicators marking hidden rows.",
		],
