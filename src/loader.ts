...
// GSD_PKG_ROOT — absolute path to the gsd-pi package root. Used by deployed extensions
...
if (missingPackages.length > 0) {
  ...
  process.stderr.write(
    ...
    `  • An outdated local build (run: git pull && pnpm install && pnpm run build:core)\n` +
    `  • A filesystem error prevented linking or copying the workspace packages\n\n` +
    `Fix it by rebuilding this fork from source:\n\n` +
